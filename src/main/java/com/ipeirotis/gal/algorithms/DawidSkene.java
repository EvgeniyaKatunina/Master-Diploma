...
public class DawidSkene {
    ...
    public DawidSkene(Set<Category> categories) {
        ...
        if (!fixedPriors) initializePriors();
        ...
        for (Datum d : this.objects.values()) {
            for (AssignedLabel al : d.getAssignedLabels()) {
                ...
                Map<String, Double> estimatedCorrectLabel =
                        d.getProbabilityVector(ClassificationMethod.DS_MaxLikelihood);
                ...
                for (String from : estimatedCorrectLabel.keySet()) {
                    ...
                    if (categoryProbability == 0.0 || Double.isNaN(labelingProbability) || labelingProbability == 0.0)
                        continue;
                    else result += Math.log(categoryProbability) + Math.log(labelingProbability);
                }
            }
        }
    ...
    public double estimate(int maxIterations, double epsilon) {
        ...
        TsvWriter workersErrorRatesWriter = null;
        try {
            workersErrorRatesWriter = new TsvWriter(new PrintWriter(new FileWriter("workers_error_rates"
                    + ".tsv")), new TsvWriterSettings());
        } catch (IOException e) {
            e.printStackTrace();
        }
        ArrayList<String[]> errorRates = new ArrayList<>();
        workersErrorRatesWriter.writeHeaders("user_id", "error_rate");
        workers.keySet().forEach(worker -> {
            Worker currentWorker = workers.get(worker);
            double error_rate = 0;
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    if (i == j) {
                    ...
                    error_rate += currentWorker.getErrorRate(Integer.toString(i), Integer.toString(j));
                }
            }
            errorRates.add(new String[]{worker, Double.toString(error_rate)});
        });
        workersErrorRatesWriter.writeRowsAndClose(errorRates.toArray(new String[errorRates.size()][2]));
        datumFieldAccessors = FieldAccessors.DATUM_ACCESSORS.getFieldAccessors(this);
    ...
    private HashMap<String, Double> getObjectClassProbabilities(String objectName, String workerToIgnore) {
        ...
        if (workerToIgnore != null && labels.size() == 1) {
            for (AssignedLabel al : labels) {
                if (al.getWorkerName().equals(workerToIgnore)) return null;
            }
        }
        ...
        for (Category category : categories.values()) {
            ...
            for (AssignedLabel al : d.getAssignedLabels()) {
                ...
                if (workerToIgnore != null && w.getName().equals(workerToIgnore)) continue;

                String assigned_category = al.getCategoryName();
                double score = Main.userId2Score.get(Integer.parseInt(al.getWorkerName()));
                boolean skip = false;
               /* double score = 1;
                try {
                    score = Main.userId2TaskId2Score.get(Integer.parseInt(al.getWorkerName())).get(Integer.parseInt(al.getObjectName()));
                } catch (NullPointerException n) {
                    skip = true;
                   // System.out.println("worker=" + al.getWorkerName() + ", task=" + al.getObjectName());
                } */
                if (!skip && !category.getName().equals(assigned_category)) {
                    score = 1 - score;
                }
                double evidence_for_category = w.getErrorRate(category.getName(), assigned_category) * score;
                if (Double.isNaN(evidence_for_category)) continue;
    ...
    private void updatePriors() {
        ...
        for (Datum d : this.objects.values()) {
            for (String c : this.categories.keySet()) {
                Double prior = priors.get(c);
                Double objectProb = d.getCategoryProbability(ClassificationMethod.DS_MaxLikelihood, c);
    ...
    private void updateWorkerConfusionMatrix(String workerName) {
        ...
        for (AssignedLabel al : w.getAssignedLabels()) {
            ...
            if (probabilities == null) continue; // No other worker labeled the object
